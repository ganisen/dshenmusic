...
- **2026-09-01 (upcoming-show card)** — Added the **04 September 2026 UNPLUGGED show at
  ...
  - **The promo Short is embedded in the card** (added the same day, owner request):
    `NkWRhwBRFlg`, 9:16, sized ~216–280px wide so the clip carries the block. **The card has
    two layouts** (owner layout call the same day): on mobile a centred poster column — date
    badge centred on top, then teaser, details one line each, full-width button; on desktop
    (≥760px) a single row across the container — date · teaser · details-over-button. Same
    DOM either way, only the grid changes. Two small generalizations of the video facade made it fit: a
